...
const ProcedureMainCollapsibleGrid = ({ procedures, title }) => {
  ...
  const isLargeScreen = useMediaQuery({ minDeviceWidth: 1133 });
  const isMediumScreen = useMediaQuery({ minDeviceWidth: 744, maxWidth: 1132 });
  const showProceduresCount = isLargeScreen ? 5 : isMediumScreen ? 4 : 2;
  const showProcedures = procedures.slice(0, showProceduresCount);
  const remainingProcedures = procedures.slice(showProceduresCount);
  const formattedTitle =
    formatTitleToFirstWord(title).charAt(0).toUpperCase() +
    formatTitleToFirstWord(title).slice(1).toLowerCase();
  ...
  const showProceduresList = showProcedures.map((name) => (
    ...
    >
      <img
        ...
        className="procedure-main-icon-pic"
        alt={name}
      />
  ...
  return (
    <div>
      ...
      <div className="procedure-main-icons-container">{showProceduresList}</div>
